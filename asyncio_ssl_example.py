...
def main():
    ...
    assert os.path.isfile('certdata/player-0.cert')
    assert os.path.isfile('certdata/player-0.key')
    assert os.path.isfile('certdata/player-1.cert')
    assert os.path.isfile('certdata/player-1.key')
...
def server():
    ...
    sc = ssl.create_default_context(ssl.Purpose.CLIENT_AUTH, cafile='certdata/player-1.cert')
    sc.verify_mode = ssl.CERT_REQUIRED
    sc.load_cert_chain('certdata/player-0.cert', 'certdata/player-0.key')
...
def client():
    ...
    def tcp_echo_client(loop):
        sc = ssl.create_default_context(ssl.Purpose.SERVER_AUTH,
            cafile='certdata/player-0.cert')
        sc.verify_mode = ssl.CERT_REQUIRED
        sc.load_cert_chain('certdata/player-1.cert', 'certdata/player-1.key')

        reader, writer = yield from asyncio.open_connection(
            'hbmpc_0', port, ssl=sc, loop=loop)
        writer.write(b'ping\n')
        yield from writer.drain()
        data = yield from reader.readline()
        assert data == b'pong\n', repr(data)

    ...
    def sqrt(self):
        ...
        if self.modulus % 4 == 3:
            ...
            for i in range(1, self.modulus):
                ...
                u = i*i - self
                # if pow(u, (self.modulus-1)//2, self.modulus) == self.modulus - 1:
                if pow(u, (self.modulus-1)//2) == self.modulus - 1:
                    t = i
                    break

            def cipolla_mult(a, b, w):
                # return (GFElement((a[0]*b[0] + a[1]*b[1]*w), self.field), \
                #         GFElement((a[0]*b[1] + a[1]*b[0]), self.field))
                return ((a[0]*b[0] + a[1]*b[1]*w), (a[0]*b[1] + a[1]*b[0]))

            exp = (self.modulus+1)//2
            exp = [int(x) for x in bin(exp)[2:]]
            x1 = (t, 1)
